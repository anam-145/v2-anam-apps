...
class MnemonicFlowManager {
  ...
  async skipAndCreateWallet() {
    try {
      ...
      localStorage.setItem('eth_wallet_status', 'pending_backup');
  ...
  async completeFlow() {
    try {
      ...
      localStorage.setItem(walletKey, JSON.stringify(walletData));
      localStorage.setItem('eth_wallet_status', 'active');
      
      // 스킵 카운트 초기화
      localStorage.removeItem('eth_mnemonic_skip_count');
      localStorage.removeItem('eth_mnemonic_skipped');
  ...
  // 백업 리마인더 체크
  checkBackupReminder() {
    const walletStatus = localStorage.getItem('eth_wallet_status');
    const skipCount = parseInt(localStorage.getItem('eth_mnemonic_skip_count') || '0');
    
    if (walletStatus === 'pending_backup' && skipCount > 0) {
      // 스킵 횟수에 따라 리마인더 표시 주기 결정
      const lastReminder = localStorage.getItem('eth_last_backup_reminder');
      ...
      if (!lastReminder || (now - parseInt(lastReminder)) > reminderInterval) {
        ...
        localStorage.setItem('eth_last_backup_reminder', now.toString());
      }
    }
  }
