// ================================================================
// Rwanda Digital Currency Adapter 구현
// RwandaAdapter 클래스만 포함 (공통 코드는 템플릿 참조)
// ================================================================

// Rwanda 어댑터 설정 (네트워크 설정은 utils/config.js에서 가져옴)
const RwandaAdapterConfig = {
  // 기본 정보
  name: "Rwanda Franc",
  symbol: "RWF",
  decimals: 2, // Rwanda Franc는 2 decimals (통화 표준)
  
  // 네트워크 설정 (config.js에서 동적으로 가져옴)
  network: {
    get networkName() {
      return window.RwandaConfig?.getActiveNetwork() || "mainnet";
    },
    get apiBaseUrl() {
      return window.RwandaConfig?.apiBaseUrl || "https://api.bnr.rw/digital";
    },
    get explorerUrl() {
      return window.RwandaConfig?.explorerUrl || "https://explorer.bnr.rw";
    }
  },
  
  // UI 테마 설정
  theme: {
    primaryColor: "#00A1DE", // 르완다 국기 블루
    secondaryColor: "#FBD016", // 르완다 국기 노란색
    logoText: "Rwanda Digital",
  },
  
  // 주소 설정
  address: {
    // 주소 형식 정규식 (Rwanda Digital Currency 형식)
    regex: /^(RW[a-zA-Z0-9]{32,40})$/,
    // 주소 표시 형식
    displayFormat: "RW...", // Rwanda Digital 형식
  },
  
  // 트랜잭션 설정 (config.js에서 가져옴)
  transaction: {
    get feeRate() {
      return window.RwandaConfig?.TRANSACTION?.DEFAULT_FEE_RATE || 50;
    },
    get minAmount() {
      return window.RwandaConfig?.TRANSACTION?.MIN_AMOUNT_RWF || "100.00";
    },
    get confirmationTime() {
      return window.RwandaConfig?.TRANSACTION?.CONFIRMATION_TIME || 3000;
    }
  },
...
// 설정 접근자
window.CoinConfig = RwandaAdapterConfig;
...
// RwandaAdapter 인스턴스 생성 및 설정
const rwandaAdapter = new RwandaAdapter(RwandaAdapterConfig);
window.setAdapter(rwandaAdapter);

console.log('[RwandaAdapter] Module loaded');
