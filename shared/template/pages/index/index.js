...
// ================================================================
// TODO: BitcoinConfig를 해당 블록체인 Config로 변경
// 예시:
// - Ethereum: const { CACHE, getCurrentNetwork } = window.EthereumConfig || {};
// - Solana: const { CACHE, getCurrentNetwork } = window.SolanaConfig || {};
// - Cosmos: const { CACHE, getCurrentNetwork } = window.CosmosConfig || {};
...
// 예시:
// - Ethereum: const { showToast } = window.EthereumUtils || {};
// - Solana: const { showToast } = window.SolanaUtils || {};
...
async function fetchTransactionHistory(address) {
  ...
  // ================================================================
  // TODO: 블록체인별 pending 트랜잭션 관리 로직 구현
  // 1. localStorage에 pending TX 저장
  // 2. API 결과와 비교하여 확정된 TX 제거
  // 3. 만료 시간 처리 (블록체인별 mempool 만료 시간)
  //    - Bitcoin: 14일
  //    - Ethereum: 12시간
  //    - Solana: 2분 (150 slots)
  
  const cacheKey = `btc_tx_${address}`; // TODO: 키 이름 변경
...
function createTransactionElement(tx, isSent) {
  ...
  if (tx.isPending) {
    ...
    // TODO: 블록체인별 단위 변환 함수 사용
    // 예시:
    // - Ethereum: ethers.parseEther(tx.amount)
    // - Solana: tx.amount * LAMPORTS_PER_SOL
    ...
    // TODO: 블록체인별 트랜잭션 금액 계산 로직
    // 예시:
    // - Bitcoin: UTXO 모델 (inputs/outputs 처리)
    // - Ethereum: Account 모델 (value 필드만)
    // - Solana: preBalance/postBalance 차이
...
function analyzeQRData(data) {
  ...
  // Bitcoin 예시: Bech32, P2PKH, P2SH 형식
