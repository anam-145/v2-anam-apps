...
function deleteWallet() {
  try {
    ...
    // TODO: 블록체인별 캐시 삭제
    // ================================================================
    // CoinConfig.symbol을 사용하거나 직접 prefix 지정:
    // - Bitcoin: 'btc_'
    // - Ethereum: 'eth_'
    // - Solana: 'sol_' 또는 'solana_'
    // - Cosmos: 'cosmos_'
    // - Sui: 'sui_'
    const prefix = CoinConfig.symbol.toLowerCase() + '_';

    // Clear ALL blockchain-related caches comprehensively
    const keysToRemove = [];
    ...
    // Remove all blockchain-related items
    ...
    if (wallet && wallet.address) {
      // 공통 캐시
      localStorage.removeItem(`${prefix}tx_${wallet.address}`);
      localStorage.removeItem(`${prefix}balance_${wallet.address}`);

      // TODO: 블록체인별 추가 캐시 삭제
      // Solana의 경우:
      // localStorage.removeItem(`${prefix}tokens_${wallet.address}`);

      // Cosmos의 경우:
      // localStorage.removeItem(`${prefix}delegation_${wallet.address}`);

      // Sui의 경우:
      // localStorage.removeItem(`${prefix}objects_${wallet.address}`);
    }
    ...
    localStorage.removeItem(`${prefix}tx_pending`);
    localStorage.removeItem(`${prefix}balance_pending`);

    // ================================================================
    // TODO: 네트워크 설정 삭제
    // ================================================================
    // Clear network settings (블록체인별로 키 이름이 다를 수 있음)
    localStorage.removeItem(`${prefix}active_network`);
    // 또는 (Solana2/Sui2/Cosmos2 패턴):
    // localStorage.removeItem(`${CoinConfig.symbol.toLowerCase()}_active_network`);
