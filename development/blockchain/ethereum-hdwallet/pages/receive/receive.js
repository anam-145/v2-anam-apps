...
function updateUI() {
  ...
  // 주소 표시 - HD Wallet 시스템 사용
  const accountInfo = window.App.getCurrentAccountInfo();
  if (accountInfo && accountInfo.address) {
    document.getElementById('receive-address').textContent = accountInfo.address;
  } else {
    document.getElementById('receive-address').textContent = 'No wallet found';
  }
}
