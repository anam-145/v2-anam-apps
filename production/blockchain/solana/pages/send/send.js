...
// ================================================================
// Solana Utils 임포트
const { showToast, formatBalance, validateAmount, validateAddress } = window.SolanaUtils || {};
...
async function confirmSend() {
  ...
  let balance;
  try {
    balance = await adapter.getBalance(currentWallet.address);
  } catch (err) {
    console.error("Failed to fetch balance:", err);
    showToast && showToast("Failed to fetch balance", "error");
    return;
  }

  const amountLamports = window.SolanaUtils?.solToLamports(amount) || Math.floor(amountValue * 1000000000);
  const balanceLamports = parseInt(balance);
  ...
  try {
    // Solana는 고정 수수료 (5000 lamports)

    // 최소 금액 검증
    const minLamports = 1;

    if (amountLamports < minLamports) {
      const minSOL = (minLamports / 1000000000).toFixed(9);
      showToast && showToast(`Minimum amount is ${minSOL} SOL`, "error");
      return;
    }
    ...
    const pendingTx = {
      signature: result.hash,  // Solana uses signature
      from: currentWallet.address,
      to: recipient,
      amount: parseFloat(amount),  // SOL 단위로 저장
    ...
    // 기존 캐시 가져오기
    const cacheKey = `sol_tx_${currentWallet.address}`;
    ...
    // Pending 트랜잭션 플래그 설정
    localStorage.setItem('sol_has_pending_tx', 'true');
    localStorage.setItem('sol_pending_start_time', Date.now().toString());
...
function handleQRScanned(event) {
  ...
  if (event.detail && event.detail.success) {
    ...
    // Solana 주소 형식 확인 (Base58, 32-44자)
    if (qrData && qrData.match(/^[1-9A-HJ-NP-Za-km-z]{32,44}$/)) {
      // 주소 필드에 입력
      ...
      console.error("Invalid Solana address format:", qrData);
      showToast && showToast("Invalid address format", "error");
    }
  } else {
