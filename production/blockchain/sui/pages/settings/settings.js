...
// Utils 함수 가져오기
const { showToast, copyToClipboard: copyToClipboardUtil } = window.SuiUtils || {};
...
function deleteWallet() {
  try {
    ...
    // Clear ALL Sui-related caches comprehensively
    const keysToRemove = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith('sui_')) {
        keysToRemove.push(key);
      }
    }

    // Remove all Sui-related items
    ...
    if (wallet && wallet.address) {
      localStorage.removeItem(`sui_tx_${wallet.address}`);
    }
...
function showNetworkSelector() {
  ...
  const currentNetworkId = window.SuiConfig?.getActiveNetwork() || 'testnet';
...
async function selectNetwork(networkId) {
  ...
  try {
    ...
    const success = window.SuiConfig.setActiveNetwork(networkId);
    ...
    window.SuiConfig.clearNetworkCache();
    ...
    // Sui는 모든 네트워크에서 동일한 주소를 사용하므로 단순히 네트워크 설정만 변경
    if (currentWalletData) {
      console.log("[selectNetwork] Switching to network:", networkId);
