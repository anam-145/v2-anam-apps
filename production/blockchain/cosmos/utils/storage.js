// ================================================================
// Cosmos2 - Wallet Storage Manager
...
(function() {
  ...
  window.WalletStorage = {
    ...
    // Storage 키
    KEYS: {
      storage: 'cosmos_wallet',
      session: 'cosmos_wallet_cache'
    },

    /**
     * 지갑 데이터 가져오기
     * 우선순위: 메모리 > sessionStorage > localStorage
     * Cosmos는 네트워크와 관계없이 동일한 주소 사용
     */
    get: function() {
      // 1. 메모리 캐시 확인
      if (this.wallet) {
        // Cosmos는 네트워크 동기화 불필요
        return this.wallet;
      }
      ...
      try {
        ...
        if (cached) {
          ...
          // Cosmos는 네트워크별 주소가 동일하므로 동기화 불필요
          // SessionStorage 업데이트
  ...
  console.log('[Cosmos WalletStorage] Module loaded with Keystore API support');
})();