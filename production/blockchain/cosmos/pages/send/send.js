...
const { showToast, formatBalance, validateAmount, validateAddress } = window.CosmosUtils || {};
    ...
    if (currentWallet && adapter) {
      try {
        ...
        const formattedBalance = formatBalance ?
          ...
          (parseInt(balance) / 1000000).toFixed(6);
...
async function confirmSend() {
  ...
  // 잔액 검증
  const balance = await adapter.getBalance(currentWallet.address);
  const amountUatom = window.CosmosUtils?.atomToUatom(amount) || Math.floor(amountValue * 1000000);
  ...
  try {
    ...
    // Cosmos는 gas 기반 수수료 사용
    if (feeLevel && feeRate) {
      txParams.gasPrice = parseInt(feeRate);
    }
    ...
    const pendingTx = {
      txid: result.hash,  // Cosmos uses txid instead of hash
      from: currentWallet.address,
      to: recipient,
      amount: parseFloat(amount),  // ATOM 단위로 저장
    ...
    // 기존 캐시 가져오기
    const cacheKey = `cosmos_tx_${currentWallet.address}`;
    ...
    // Pending 트랜잭션 플래그 설정
    localStorage.setItem('cosmos_has_pending_tx', 'true');
    localStorage.setItem('cosmos_pending_start_time', Date.now().toString());
...
function handleQRScanned(event) {
  ...
  if (event.detail && event.detail.success) {
    ...
    // Cosmos 주소 형식 확인 (bech32 형식)
    if (qrData && qrData.match(/^cosmos1[a-z0-9]{38}$/)) {
      // 주소 필드에 입력
      ...
      console.error("Invalid Cosmos address format:", qrData);
      showToast && showToast("Invalid address format", "error");
    }
  } else {
