...
// Utils 함수 가져오기
const { showToast, copyToClipboard: copyToClipboardUtil } = window.CosmosUtils || {};
...
function deleteWallet() {
  try {
    ...
    // Clear ALL Cosmos-related caches comprehensively
    const keysToRemove = [];
    ...
    // Remove all Cosmos-related items
    ...
    if (wallet && wallet.address) {
      localStorage.removeItem(`cosmos_tx_${wallet.address}`);
      localStorage.removeItem(`cosmos_balance_${wallet.address}`);
      localStorage.removeItem(`cosmos_delegation_${wallet.address}`);
    }

    // Clear any pending flags
    localStorage.removeItem('cosmos_tx_pending');
    localStorage.removeItem('cosmos_balance_pending');

    // Clear network settings
    localStorage.removeItem('cosmos_active_network');
...
function showNetworkSelector() {
  ...
  const currentNetworkId = window.CosmosConfig?.getActiveNetwork() || 'testnet';
...
async function selectNetwork(networkId) {
  ...
  try {
    ...
    const success = window.CosmosConfig.setActiveNetwork(networkId);
    ...
    window.CosmosConfig.clearNetworkCache();
