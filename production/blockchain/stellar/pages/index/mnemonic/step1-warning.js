// Stellar Mnemonic Flow - Step 1: Security Warning
...
class SecurityWarningStep {
  ...
  handleSkip() {
    const prefix = (window.CoinConfig?.symbol || 'XLM').toLowerCase();
    const skipCountKey = `${prefix}_mnemonic_skip_count`;
    const skippedKey = `${prefix}_mnemonic_skipped`;

    // Increment skip count
    const skipCount = parseInt(localStorage.getItem(skipCountKey) || '0', 10) + 1;
    localStorage.setItem(skipCountKey, skipCount.toString());
