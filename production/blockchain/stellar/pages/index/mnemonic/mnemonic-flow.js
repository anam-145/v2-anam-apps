...
class MnemonicFlowManager {
  ...
  async generateWallet() {
    try {
      ...
      if (!adapter) {
        throw new Error('Stellar adapter not found');
      }
...
if (typeof window !== 'undefined') {
  ...
  // 전역 함수 등록
  window.startMnemonicFlow = function() {
    if (window.mnemonicFlow) {
      window.mnemonicFlow.start();
    } else {
      console.error('[MnemonicFlow] Manager not initialized');
    }
  };
